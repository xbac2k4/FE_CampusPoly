...
const HomeScreen = ({ navigation, route }) => {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      // console.log("filteredDataAdmin:", filteredDataAdmin); // Kiểm tra dữ liệu đã được lọc
      // Kiểm tra nếu không có phần tử nào sau khi lọc
      if (filteredDataAdmin.length === 0) {
        console.log("Không có bài viết admin nào.");
      }
      // Cập nhật slider
      const adminPages = filteredDataAdmin.map(item => ({
